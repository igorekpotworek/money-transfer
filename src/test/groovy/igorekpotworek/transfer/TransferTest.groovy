...
class TransferTest extends Specification {
    ...
    def "should perform successful transfer"() {
        given: "two accounts"
        def sourceAccountId = client.toBlocking().exchange(POST(accountUri, new AccountDto(1000.0)), Account).body().getId()
        def destinationAccountId = client.toBlocking().exchange(POST(accountUri, new AccountDto(2000.0)), Account).body().getId()
        ...
        def result = client.toBlocking().exchange(POST(transferUri, transferDto))
    ...
    def "should return error when there is insufficient balance to perform transfer"() {
        ...
        client.toBlocking().exchange(POST(transferUri, transferDto))
    ...
    def "should return not found error when try to transfer money to non existing account"() {
        ...
        def sourceAccountId = client.toBlocking().exchange(POST(accountUri, new AccountDto(2000.0)), Account).body().getId()
